...
- 发布保持不可变 release、数据库备份、最小健康门禁和上一版本即时回滚能力。
...
- 按用户最终要求直接更新正式服务，不执行候选端口或额外页面测试；仅保留构建、迁移、备份、容器健康与 API 健康等部署必要门禁。
...
- [x] 阶段五：使用 Workbench 完成服务器审计、镜像构建、数据库迁移与备份，并直接更新正式服务。
- [x] 阶段六：记录发布 SHA、回滚版本与健康结果，归档任务文档。

## 实施结果

- 本地仅保留 `master`，远程仅保留 `origin/master`，`origin/HEAD` 指向 `origin/master`；原 `codex/extract-love-project` 本地和远程引用均已删除。
- 代码与治理文档先收敛到 `7c84e81e94c180401c884b41e64d7de74b1db467`，本地 `npm run check` 已通过：lint、类型检查、204 项测试（203 通过、1 项既有 PostgreSQL 集成测试按环境跳过）和生产构建均成功。
- 服务器从 `origin/master` 创建不可变 release，构建 `love:7c84e81e94c180401c884b41e64d7de74b1db467`，完成数据库迁移并生成非空备份；上一 release `e1cf7994834b69063fc959dd31b39175c9c7a491` 保留用于回滚。
- `/srv/love/current` 已切换至新 release，`love-app-1` 使用新镜像且容器健康状态为 `healthy`，正式端口健康接口返回数据库和对象存储均正常。
- 本文档归档提交只变更 `.agentdocs`。归档后服务器再拉取唯一的最新 `master`，复用完全相同的运行镜像内容建立最终 release，使本地、远程和服务器 HEAD 再次对齐。
...
- 按用户要求不执行额外页面测试；正式端口 `/api/health` 作为部署必要门禁返回成功。
