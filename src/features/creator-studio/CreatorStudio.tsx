          ...
          <div className="studio-panel__body">
            ...
            {step === 'ambience' && (
              <StudioSection eyebrow="04 · ATMOSPHERE" title="选择你想留下的时刻">
                ...
                <Field label="分享体验"><select value={config.experience.immersiveEnabled ? 'immersive' : 'free'} onChange={(event) => patchConfig('experience', { ...config.experience, immersiveEnabled: event.target.value === 'immersive' })}><option value="immersive">沉浸浪漫（推荐）</option><option value="free">直接自由享受</option></select></Field>
                <Field label="沉浸结尾"><textarea rows={3} value={config.experience.endingMessage} maxLength={120} onChange={(event) => patchConfig('experience', { ...config.experience, endingMessage: event.target.value })} /></Field>
